...
export const store = new Vuex.Store({
  ...
  mutations: {
    savePrefs(state, { name, prefs }) {
      ...
      state.user.activities.unshift({ txt: 'Changed prefs', at: Date.now() })
      userService.save(JSON.parse(JSON.stringify(state.user)))
    },
    addTodo(state, { todo }) {
      var newTodo = todoService.save(todo)
      state.todos.unshift(newTodo)
      state.user.activities.unshift({ txt: 'Added a Todo', at: Date.now() })
      userService.save(JSON.parse(JSON.stringify(state.user)))
    },
    deleteTodo(state, { id }) {
    ...
    updateTodo(state, { todo }) {
      ...
      state.todos = todoService.query(state.filterBy)
      state.user.activities.unshift({ txt: 'Updated a Todo', at: Date.now() })
      userService.save(JSON.parse(JSON.stringify(state.user)))
    },
    toggleTodo(state, { todo }) {
      ...
      todoService.save({ ...todo })
      state.todos = todoService.query(state.filterBy)
      state.user.activities.unshift({ txt: 'Toggled a Todo', at: Date.now() })
      userService.save(JSON.parse(JSON.stringify(state.user)))
    },
